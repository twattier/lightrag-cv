...
3. **Submit to LightRAG** with metadata headers:
   ...
   async def submit_cv(cv_meta: Dict, parsed_data: Dict):
       ...
       print(f"✓ Submitted {len(texts)} chunks for {candidate_label}")
   ```
...
5. **Handle `--candidate-label` parameter**:
   ...
   if args.candidate_label:
       ...
       if not cvs_to_import:
           print(f"❌ CV not found: {args.candidate_label}")
           sys.exit(1)
   else:
       cvs_to_import = cvs
   ```
...
```
/data/cvs/
├── docs/                     # Downloaded CV PDFs (cv_001.pdf, cv_002.pdf, ...)
├── parsed/                   # Parsed JSON outputs (cv_001_parsed.json, ...)
└── cvs-manifest.json         # Master manifest with classifications
```
...
- ✅ Four clean, focused scripts replace existing mixed workflow
- ✅ cv1_download supports `--max-cvs` parameter for flexible dataset size
- ✅ cv2_parse runs with proper retry logic and timeout handling
- ✅ cv3_classify uses LLM abstraction layer correctly
- ✅ cv4_import submits CVs to LightRAG with metadata tracking
- ✅ All scripts use centralized config from `app.shared.config`
- ✅ Complete workflow tested: download → parse → classify → import
...
- Verify end-to-end workflow: download → parse → classify → import
...
- ✅ Config updated with `CV_DOCS_DIR` and `CV_PARSED_DIR` settings
- ✅ Fixed hardcoded `/home/wsluser/` path to use dynamic `Path(__file__)` resolution
- ✅ cv1_download.py: Added `--max-cvs` parameter, updated to use settings paths
- ✅ cv2_parse.py: Added retry logic with exponential backoff, updated to use settings
- ✅ cv3_classify.py: Changed `OLLAMA_LLM_MODEL` to `LLM_MODEL`, updated paths
- ✅ cv4_import.py: Created from scratch following CIGREF pattern with proper async/PostgreSQL/LightRAG integration
- ⚠️ Manual testing (Task 6) requires services running - left for user to complete
- 📝 cv1_download.py test initiated but HuggingFace download takes time
